...
class TransformerModel(pl.LightningModule):
    def __init__(self, model_name="dumitrescustefan/bert-base-romanian-cased-v1", lr=2e-05,
                 model_max_length=512, bio2_tag2id=[], bio2_id2tag=[], tag_list=[]):
        super().__init__()

        print("Loading AutoModel [{}] ...".format(model_name))
        self.model_name = model_name
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, strip_accents=False)
        self.model = AutoModel.from_pretrained(model_name, num_labels=len(bio2_id2tag))

        hidden_size = self.get_hidden_size() # sometimes config.hidden_size is not set, so get it manually
        print(f"\tDetected hidden size is {hidden_size}")

        self.dropout = nn.Dropout(0.1)
        self.classifier = nn.Linear(hidden_size, len(bio2_id2tag))

        self.lr = lr
        self.model_max_length = model_max_length
        self.bio2_tag2id = bio2_tag2id
        self.bio2_id2tag = bio2_id2tag
        self.tag_list = tag_list
        self.num_labels = len(bio2_id2tag)
        ...
        if self.tokenizer.cls_token_id is None:
            print(f"*** Warning, tokenizer {model_name} has no defined CLS token: sequences will not be marked with special chars! ***")
        if self.tokenizer.sep_token_id is None:
            print(f"*** Warning, tokenizer {model_name} has no defined SEP token: sequences will not be marked with special chars! ***")
       
        # add pad token if not present
        self.validate_pad_token()

        self.loss_fct = nn.CrossEntropyLoss(ignore_index=len(self.bio2_tag2id))
    ...
    def forward(self, input_ids, attention_mask, labels):
        ...
        last_hidden_state = output["last_hidden_state"] # [batch_size, seq_size, hidden_size]
        last_hidden_state = self.dropout(last_hidden_state)
        logits = self.classifier(last_hidden_state) # [batch_size, seq_size, num_labels]
...
class MyCollator(object):
    def __init__(self, tokenizer, max_seq_len, bio2_tag2id):
        self.tokenizer = tokenizer
        self.max_seq_len = max_seq_len
        self.bio2_tag2id = bio2_tag2id
        self.validate_pad_token()
        self.o_class_id = bio2_tag2id['O']
    ...
    def __call__(self, input_batch):
        ...
        return {
            ...
            "labels": torch.nn.utils.rnn.pad_sequence(batch_labels, batch_first=True, padding_value=len(self.bio2_tag2id)),
...
def run_evaluation(
    ...
    if dataset_name == "":
        ...
        print(dataset)
        sys.exit(0) # not implemented yet, this script is for testing files only

    # deduce bio2 tag mapping and simple tag list, required by nervaluate
    tags = []  # tags without the B- or I- prefix
    bio2_tag2id = {} # tags with the B- and I- prefix, all tags are here
    bio2_id2tag = [0]*31
    ...
    print(f"Dataset contains {len(bio2_id2tag)} BIO2 classes: {bio2_id2tag}.")
    tags = sorted(list(set([tag[2:] if len(tag)>2 else tag for tag in bio2_id2tag]))) # skip B- and I-
    ...
    my_collator = MyCollator(tokenizer=tokenizer, max_seq_len=model_max_length, bio2_tag2id=bio2_tag2id)
    ...
    print("\nTrain dataset has {} instances.".format(len(train_dataset)))
    ...
    while itt < experiment_iterations:
        ...
        model = TransformerModel(
            ...
            bio2_tag2id=bio2_tag2id,
            bio2_id2tag=bio2_id2tag,
            tag_list=tags
        )
    ...
    with open("results_{}.json".format(automodel_name.replace("/", "_")), "w") as f:
